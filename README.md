...
* **"Meet the Members" Carousel:** A small rotating card cycles through all 5 ITZY members one at a time — a large, clearly-visible real photo (220px tall, with the width left as `auto` so the picture frame hugs each photo's own actual shape instead of leaving empty letterboxed bars around a forced square), her full name, and her well-known position(s), each in her own accent color — auto-advancing every 5 seconds with Prev/Next arrows and dot navigation for manual browsing. Auto-advancing is skipped entirely for anyone with `prefers-reduced-motion` set — it only ever moves when they click it themselves. Each name's color is mixed with a touch of black (light mode) or white (dark mode) rather than used raw, the same readability trick as the profile badges, since a few eras land close enough to one of the five accent colors that the raw color alone gets hard to read.
