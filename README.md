...
* **Full 31-Song Moodboard, Every Photo and Link Real:** Every ITZY release has its own photocard — not just the 17 main comebacks, but every pre-release, Japanese version, and collab too — all in chronological order from 2019's "DALLA DALLA" to 2026's "MOTTO," each with a real photo and a real official YouTube link (no placeholders left). Only the 17 *main* comebacks (the ones with proper music-show promotion) are also options in the era color-theme dropdown — the pre-releases/Japanese versions/collabs are extra photocards for completeness, not full "eras." On wide screens, the two photocard columns run down either side of the card in a simple top-to-bottom stack with a gentle alternating side-to-side zigzag (rather than fully randomized scatter), which guarantees no two cards ever overlap however many songs get added later — see "Concepts Demonstrated" below for why this replaced an earlier absolute-positioned version. Each photocard is a genuine link (`<a>`) that opens in a new tab; each one also carries its own `aria-label` (e.g. "Watch the DALLA DALLA music video on YouTube") for screen readers. Photos use `object-fit: contain` so nothing gets cropped, however tall or wide the source image is. None of the photos on the page use `loading="lazy"` — that attribute can misfire on a layout this dynamic, so every image is set to load immediately instead, trading a small amount of best-practice performance for guaranteed reliability across devices.
* **Blurred Background Photo Wall, Uncropped:** 9 additional photos tile into a fixed, full-viewport 3×3 grid behind everything, heavily blurred and tinted so they read as ambient texture rather than a distracting gallery. Each photo uses `object-fit: contain` (not `cover`), so the whole picture always fits inside its grid square instead of getting zoomed in and cropped — this matters most on tall, narrow phone screens, where a "cover" fit was chopping most of every photo off to fill its skinny grid cell. Any leftover empty sliver around a photo is filled with the page's own current `--bg` color, so it still blends in smoothly.
...
* **Knowing When to Abandon an Approach That Stopped Scaling:** An earlier version of the desktop moodboard hand-placed all 31 photocards at exact percent-based positions (via absolute positioning + `nth-child` rules), stretched to always match the card + footer's height so the layout could never force extra scrolling. That worked at 17 cards, but once the timeline grew to 31 real photos, the math ran out of vertical room and captions started overlapping — which made them unreadable. The fix wasn't to hand-tune 31 more magic numbers; it was to recognize the underlying assumption ("everything must fit in a fixed height") no longer held, and switch to a normal auto-height flex column instead — trading "guaranteed no extra scroll" for "guaranteed no overlap, ever, no matter how many songs get added later." A small alternating `margin-left` per card keeps a bit of the original "scattered" character without any overlap risk, since horizontal nudges never affect how much vertical space a card takes up.
